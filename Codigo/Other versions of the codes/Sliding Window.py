# -*- coding: utf-8 -*-
"""
In this script we will do a first approach for deetecting the LEFT, RIGHT and PASS
...
with open('RightPass_Classification.pkl', 'rb') as f:
    clf3 = pickle.load(f)
with open('LeftRight_Classification_More_Data.pkl', 'rb') as f:
    clf4 = pickle.load(f)
#load scaler
...
DL_model_Pass_detection = tf.keras.models.load_model('Pass_vs_Left_Right_with_DL.h5')

with open('Pass_vs_Left_Right_with_DL_Scaler.pkl', 'rb') as f:
...
for i in np.arange(2500, 2700, 0.1):
    vala = i
    # Remember that the sampling frequency is 200. The 180 are the samples between 2.1 and 3 second (0.9*200)
    ACA = [Acisco[:, int(vala*200):int(vala*200+180)]]
    ACA_All = [AciscoAll[:, int(vala*200):int(vala*200+180)]]
    ...
    if pred == [1] and clf1.predict_proba(FEAT)[0,0]>0.5: # we must be 70% sure that the prediction is okay.
        if i > (times[0]):
...
for i in np.arange(500, 1000, 0.1):
    ...
    # Remember that the sampling frequency is 200. The 180 are the samples between 2.1 and 3 second (0.9*200)
    ACA = [Acisco[:, int(vala*200):int(vala*200+180)]]
    ACA_All = [AciscoAll[:, int(vala*200):int(vala*200+180)+1]]
    CA_skw, CA_krt, CA_energy, CA_entropy, CA_shannon, CA_log = feature_extracter(
        ACA)
    FEAT = feature_ex(CA_skw, CA_krt, CA_energy,
                      CA_entropy, CA_shannon, CA_log)
...
for i in np.arange(2500, 2700, 0.1):
    vala = i
    # Remember that the sampling frequency is 200. The 180 are the samples between 2.1 and 3 second (0.9*200)
    ACA = [Acisco[:, int(vala*200):int(vala*200+180)+1]]
    ACA_All = [AciscoAll[:, int(vala*200):int(vala*200+180)]]
    ...
    FEAT = np.abs(Zx)
    pred = [int((DL_model_Pass_detection.predict(Pass_scaler.transform(np.array(FEAT).reshape(1,3*91*181)).reshape(1,3,91,181))> 0.5).astype("int32"))+1]
